...
func cpuCard(c collector.CPUStat, hist []float64, innerWidth int, focused bool) string {
	sparkW := clampInt(innerWidth/2, 8, maxInt(innerWidth-8, 8))
	header := sparkline(sparkW, hist, colBlue)

	n := len(c.PerCore)
	if n == 0 {
		return renderCard(innerWidth, "▣", "CPU", header, []string{faintStyle.Render("无 CPU 数据")}, focused)
	}

	labelW := maxInt(len(strconv.Itoa(n-1)), 1)
	const pctW = 5 // "100.0"
	meterW := maxInt(innerWidth-labelW-1-1-pctW, 1)
	...
	return renderCard(innerWidth, "▣", "CPU", header, lines, focused)
}
